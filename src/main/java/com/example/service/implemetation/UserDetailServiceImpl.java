...
public class UserDetailServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        UserEntity userEntity = userRepository.
                findUserEntityByUsername(username).orElseThrow(() -> new UsernameNotFoundException("El usuario " + username + " no existe."));
        ...
        userEntity.getRoles().stream().flatMap(role -> role.getPermissionList()
                .stream()).forEach(permission -> authorityList.add(new SimpleGrantedAuthority(permission.getName())));
    ...
    public AuthResponse loginUser(AuthLoginRequest authLoginRequest) {
        ...
        AuthResponse authResponse = new AuthResponse(username, "Usuario logueado exitosamente", accessToken, true);
        return authResponse;
    }
